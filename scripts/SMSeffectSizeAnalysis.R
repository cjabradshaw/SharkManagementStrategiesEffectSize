...
## February 2023 / updated October 2023                                    ##
#############################################################################
...
#################################
# choose dataset
bm.dat <- read.csv("beachmesh.csv", header=T) # all data
#bm.dat <- read.csv("beachmeshonlybites.csv", header=T) # all data
head(bm.dat)
...
## add 'correction' for relative human population size
table(bm.dat$Period)
popAus <- c(19136268,19381605,19600748,19822481,20041241,20306728,20622539,21010812,21470087,21859719,22166335,
             22515901,22921637,23291216,23633762,23978081,24376709,24750353,25137059,25510998,25620615,
            25760867,26280421) # from 2000-20022 abs.gov.au
popYr <- 2000:2022
AusPop <- data.frame(popYr, popAus)
plot(AusPop$popYr, AusPop$popAus, type="b", pch=19, xlab="year", ylab="N")
periodPopMn <- c(mean(popAus[1:5]), mean(popAus[6:10]), mean(popAus[11:15]), mean(popAus[16:20]), mean(popAus[20:23]))
periodYr <- c(2002,2007,2012,2017,2021)
points(periodYr,periodPopMn, pch=1, cex=1.5, col="red")

periodAusMnpropCh <- periodPopMn[1]/periodPopMn
Period <- attr(table(bm.dat$Period),"names")
periodAusPopChange <- data.frame(Period,periodAusMnpropCh)
periodAusPopChange
bm.dat <- merge(bm.dat, periodAusPopChange, by="Period")
head(bm.dat)
...
# only 2010 onwards
#bm.dat <- bm.dat[-which(bm.dat$Period == "2000-2005" | bm.dat$Period == "2010-2015"), ]
...
for(i in 1:Modnum) {
  ...
  fit <- glm(as.formula(mod.vec[i]),family=binomial(link="logit"), weights=bm.dat$wt*bm.dat$periodAusMnpropCh, data=bm.dat, na.action=na.omit)
...
#######################################################################################################
# increase bites in 'no mesh' beaches only to examine effect on probability of detecting differences ##
#######################################################################################################
...
# weights for human population change
table(sms.dat$Year)
popAusSMS <- c(22166335,22515901,22921637,23291216,23633762,23978081,24376709,24750353,25137059,25510998,25620615,
            25760867,26280421) # from 2010-20022 abs.gov.au
popYrSMS <- 2010:2022
AusPopSMS <- data.frame(popYrSMS, popAusSMS)
plot(AusPopSMS$popYr, AusPopSMS$popAus, type="b", pch=19, xlab="year", ylab="N")
YearPrpopChSMS <- (popAusSMS[1]/popAusSMS)[-1]
YearSMS <- attr(table(sms.dat$Year), "names")
AusPopChangeSMS <- data.frame(YearSMS,YearPrpopChSMS)
colnames(AusPopChangeSMS)[1] <- "Year"
AusPopChangeSMS

sms.dat <- merge(sms.dat, AusPopChangeSMS, by="Year")
head(sms.dat)
...
#  fit <- glmer(as.formula(mod.vec[i]), family=poisson(link="log"), data=sms.dat, na.action=na.omit)
  fit <- glmer(as.formula(mod.vec[i]), family=poisson(link="log"), data=sms.dat, weights=sms.dat$YearPrpopChSMS, na.action=na.omit)
  assign(paste("fit",i,sep=""), fit)
  mod.list[[i]] <- fit
...
smsERdecr.out <- data.frame(red.vec, ER12.md, ER12.up, ER12.lo)
round(100 * (sum(post.sms$Bite)-6 - sum(pre.sms$Bite)) / sum(pre.sms$Bite), 1)
...
smsERinc.out <- data.frame(inc.vec[1:24], ER12.md[1:24], ER12.up[1:24], ER12.lo[1:24])
